...
export function createRibbon(container,ribbonConfig,store){
  function render(state){
    ...
    container.querySelectorAll('.context-button').forEach(button=>button.addEventListener('click',()=>{const row=button.dataset.row,id=button.dataset.id;store.mutate(state=>{state[row]=id;state.inspectorOpen=false;if(row==='sector'&&!state.selectedNodeBySector[id])state.selectedNodeBySector[id]=id})}));
  }
