export function createProductRepository(productIndex){
  ...
  async function loadNeed(needId){if(!needId)return[];if(cache.has(needId))return cache.get(needId);const file=files.get(needId);if(!file)return[];const promise=loadJson(file.path);cache.set(needId,promise);return promise}
