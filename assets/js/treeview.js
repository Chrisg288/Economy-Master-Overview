...
const INSPECTABLE=new Set(['product','service','model-object','record-class','classification-node','data-reference']);
...
export function createTreeView({container,countElement,filterElement,contextElement,expandButton,collapseButton,trees,store}){
  ...
  function render(state){
    ...
    container.querySelectorAll('.tree-line').forEach(line=>line.addEventListener('click',event=>{const id=line.dataset.nodeId,found=findTreeNode(root,id)?.node;store.mutate(s=>{const sector=s.sector,arr=new Set(s.expandedBySector[sector]||[]);if(event.target.classList.contains('tree-toggle')&&(found?.children||[]).length){arr.has(id)?arr.delete(id):arr.add(id);s.expandedBySector[sector]=[...arr]}else if(found?.type==='sector-link'&&found.sector){s.sector=found.sector;s.inspectorOpen=false}else{s.selectedNodeBySector[sector]=id;if((found?.children||[]).length)arr.add(id);s.expandedBySector[sector]=[...arr];s.datagrid.page=1;s.datagrid.search='';s.inspectorOpen=INSPECTABLE.has(found?.type)}})}));
  }
